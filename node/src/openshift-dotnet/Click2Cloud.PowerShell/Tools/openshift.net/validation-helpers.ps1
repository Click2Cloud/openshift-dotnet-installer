...
function Check-SQLServer2014()
{
    Write-Verbose "Checking if MS SQL Server 2014 is installed ..."

    $mssqlRegistry = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL12.MSSQLSERVER2014\Setup' -ErrorAction SilentlyContinue)

    if (($mssqlRegistry -eq $null) -or ((Test-Path $mssqlRegistry.SQLPath) -eq $false))
    {
        Write-Error "Prerequisite SQL Server 2014 is not installed. Please install this and then run this script again."
        exit 1
    }

    if ((get-service MSSQL`$MSSQLSERVER2014).Status -ne "Stopped")
    {
        Write-Error "The SQL Server 2014 service 'MSSQL`$MSSQLSERVER2014' is running. Please stop and disable the service and then run this script again."
        exit 1
    }

    $sqlServerStartMode = Get-WMIObject win32_service -filter "name='mssql`$mssqlserver2014'" -computer "." | select -expand startMode

    if ($sqlServerStartMode -ne "Disabled")
    {
        Write-Error "The SQL Server 2014 service 'MSSQL`$MSSQLSERVER2014' is not disabled. Please disable the service and then run this script again."
        exit 1
    }

    Write-Host "[OK] Prerequisite SQL Server 2014 is installed."
}
