...
namespace Click2Cloud.Openshift.Common
{
    public class ParseConfig
    {
        Dictionary<string, Dictionary<string, string>> conf = new Dictionary<string, Dictionary<string, string>>(StringComparer.InvariantCultureIgnoreCase);
        string file;

        /// <summary>
        /// Initialize an Conf file
        /// Load it if it exists
        /// </summary>
        /// <param name="file">Full path where the Conf file has to be read from or written to</param>
        ...
        public void Load()
        {
                              ...
                              }))
            {
                ...
                if (line.StartsWith("[") && line.EndsWith("]"))
                {
                    ...
                    conf[line.Substring(1, line.Length - 2)] = currentSection;
                    continue;
                }
        ...
        public string GetValue(string key, string section, string @default)
        {
            ...
            if (!conf[section].ContainsKey(key))
                return @default;

            return ((conf[section][key].Split('#')[0]).Replace('"', ' ')).Trim();
        ...
        /// Save the Conf file
        /// </summary>
        /// <param name="newFile">path to new config file</param>
        ...
        public void WriteValue(string key, string section, string value)
        {
            ...
            if (!conf.ContainsKey(section))
            {
                ...
                conf.Add(section, currentSection);
            }
            else
                currentSection = conf[section];
        ...
        public string[] GetKeys(string section)
        {
            ...
            return conf[section].Keys.ToArray();
        }

        /// <summary>
        /// Get all the section names of the conf file
        /// </summary>
        /// <returns></returns>
        public string[] GetSections()
        {
            return conf.Keys.Where(t => t != "").ToArray();
        }
    }
}
