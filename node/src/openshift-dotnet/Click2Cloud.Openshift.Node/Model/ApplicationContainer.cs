...
using Click2Cloud.Openshift.Runtime.Model;
...
using System.Net;
using Click2Cloud.Openshift.Common.JsonHelper;
using System.Runtime.CompilerServices;

[assembly: InternalsVisibleTo("Click2Cloud.Openshift.Tests")]
...
namespace Click2Cloud.Openshift.Runtime
{
    public partial class ApplicationContainer
    {
        ...
        public List<RubyHash> WithGearRotation(RubyHash options, GearRotationCallback action)
        {
            ...
            // TODO: RGen: verify if this is needed for scalable apps
        ...
        public RubyHash RotateAndYield(object targetGear, Dictionary<string, string> localGearEnv, RubyHash options, GearRotationCallback action)
        {
            ...
            // TODO: RGen: check if this condition also needs boolean verification on the value in the hash
        ...
        internal void InitializeHomedir(string baseDir, string homeDir)
        {
            ...
            //RGen: Code added by Shrutika M on 21/05/2015 to store prison user password
            try
            {
                Prison.PrisonUser user = user = Prison.Prison.LoadPrisonAndAttach(PrisonIdConverter.Generate(this.Uuid)).User;
                if (user != null)
                {
                    SavePrisonUserInfo("PRISON_USERNAME", user.Username);
                    SavePrisonUserInfo("PRISON_PASSWORD", user.Password);
                }
            }
            catch (Exception ex) { Logger.Error("Error occured while getting prison user password. Message {0} | StackTrace {1}", ex.Message, ex.StackTrace); }
        ...
        //RGen: Method added by Shrutika M on 20-05-2015 to fix store prison user password
        public void SavePrisonUserInfo(string key, string value)
        {
            string userVarsDir = Path.Combine(this.ContainerDir, ".prison");
            if (!Directory.Exists(userVarsDir))
                Directory.CreateDirectory(userVarsDir);
            string fileName = Path.Combine(userVarsDir, key);
            File.WriteAllText(fileName, value);
            SetRoPermissions(fileName);
        }
        ...
        {
            this.State.Value(Click2Cloud.Openshift.Runtime.State.STOPPED);
